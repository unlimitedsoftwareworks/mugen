...
wchar_t *ReadString(
  ...
  String = (wchar_t *)dmt_malloc((Length + 1) * sizeof(wchar_t));
...
int ParseToken(
  ...
  Reduction = (struct TokenStackStruct *)dmt_malloc(sizeof(struct TokenStackStruct));
  if (Reduction == NULL) return(LALRMEMORYERROR);
  Reduction->Token = (struct TokenStruct *)dmt_malloc(sizeof(struct TokenStruct));
  if (Reduction->Token == NULL) {
    dmt_free(Reduction);
    return(LALRMEMORYERROR);
    }
  Reduction->Token->ReductionRule = Rule;
  Reduction->Token->Tokens = (struct TokenStruct **)dmt_malloc(
    sizeof(struct TokenStruct *) * Grammar.RuleArray[Rule].SymbolsCount);
  if (Reduction->Token->Tokens == NULL) {
    dmt_free(Reduction->Token);
    dmt_free(Reduction);
    return(LALRMEMORYERROR);
    }
...
int Parse(
  ...
  *FirstToken = NULL;
  ...
  while (1) {
    ...
    Work = (struct TokenStackStruct *)dmt_malloc(sizeof(struct TokenStackStruct));
    if (Work == NULL) {
    ...
    Work->NextToken = NULL;
    Work->Token = (struct TokenStruct *)dmt_malloc(sizeof(struct TokenStruct));
    if (Work->Token == NULL) {
      dmt_free(Work);
      ParseCleanup(TokenStack,NULL,FirstToken);
      return(PARSEMEMORYERROR);
    ...
    if (CommentLevel > 0) {
      ...
      if (Grammar.SymbolArray[Work->Token->Symbol].Kind == SYMBOLCOMMENTEND) {
        /* Delete the Token. */
        if (Work->Token->Data != NULL) dmt_free(Work->Token->Data);
        dmt_free(Work->Token);
        dmt_free(Work);
